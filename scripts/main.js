...
imgs.forEach(function (nav) {
    ...
    navElement.setAttribute('data-store', nav.store);
    navElement.setAttribute('data-description', nav.description);
...
function mainImgLoad (imgs) {
    ...
    storeLink.setAttribute('href', imgs[0].store);
    mainText.textContent = imgs[0].description; 
};
...
navImgs.forEach(function (nav) {
    nav.addEventListener('click', function(event) {
        ...
        storeLink.setAttribute('href', nav.getAttribute('data-store'));
        mainText.textContent = nav.getAttribute('data-description');
    });
});
...
infoLinkToggle.addEventListener('click', function(event) {
    event.preventDefault();
    infoLink.classList.toggle('link-container-open');
});
